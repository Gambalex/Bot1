...
for (const file of readdirSync("./commands/internos/")) {
  if (file.endsWith(".js")) {
    ...
    let aliases;
    try {
      aliases = archivo.help.aliases.length;
    } catch (err) {}
    if (aliases > 0 && aliases !== undefined) {
      for (let i = 0; i < aliases; i++) {
        bot.comandosIN.set(archivo.help.aliases[i], archivo);
      }
      bot.comandosIN.set(archivo.help.name, archivo);
      console.log(`${archivo.help.name}.js cargado con ${aliases} alias`);
    } else {
      console.log(`${archivo.help.name}.js cargado`);
      bot.comandosIN.set(archivo.help.name, archivo);
    }
  }
}
...
for (const file of readdirSync("./commands/informacion/")) {
  if (file.endsWith(".js")) {
    ...
    if (aliases > 0 && aliases !== undefined) {
      for (let i = 0; i < aliases; i++) {
        bot.comandosINF.set(archivo.help.aliases[i], archivo);
      }
      bot.comandosINF.set(archivo.help.name, archivo);
      console.log(`${archivo.help.name}.js cargado con ${aliases} alias`);
    } else {
      console.log(`${archivo.help.name}.js cargado`);
      bot.comandosINF.set(archivo.help.name, archivo);
